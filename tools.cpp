...
void nms_cpu(std::vector<Anchor>& boxes, float threshold, std::vector<Anchor>& filterOutBoxes) {
	...
	while (idx.size() > 0)
	{
		...
		{
			int tmp_i = tmp[i];
			float inter_x1 = max(boxes[good_idx][0], boxes[tmp_i][0]);
			float inter_y1 = max(boxes[good_idx][1], boxes[tmp_i][1]);
			float inter_x2 = min(boxes[good_idx][2], boxes[tmp_i][2]);
			float inter_y2 = min(boxes[good_idx][3], boxes[tmp_i][3]);
